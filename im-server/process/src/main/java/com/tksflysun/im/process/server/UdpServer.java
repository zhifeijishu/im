...
import com.tksflysun.im.process.handle.UdpHandler;
...
public class UdpServer {
    ...
    public void start() {
        ...
        try {
            ...
            bootstrap.group(group).channel(NioDatagramChannel.class).option(ChannelOption.SO_BROADCAST, true)
                ...
                .option(ChannelOption.RCVBUF_ALLOCATOR, new FixedRecvByteBufAllocator(20000)).handler(udpHandler);
            try {
                Channel ch = bootstrap.bind(port).sync().channel();
                channels.add(ch);
                ch.closeFuture().await();
    ...
    public static void sendMsg(Im.UdpPackage udpPackage) {
        try {
            ...
            // String ip = UserIpRedisCache.getInstance().getT(udpPackage.getToUserId() + "", String.class);
            String ip = "127.0.0.1";
            logger.warn("用户" + udpPackage.getToUserId() + "的登录ip地址为：" + ip);
            channels.get(i).writeAndFlush(
                new DatagramPacket(Unpooled.copiedBuffer(udpPackage.toByteArray()), new InetSocketAddress(ip, 8889)))
                .sync();
        } catch (Throwable e) {
            logger.warn("connect层udp消息发送失败", e);

        }
        logger.warn("process 层udp消息发送成功");
