...
public class RegistLimitRedisCache extends JedisClusterClientWrapper {
    private static final String PREFIX = "im:regist:limit:";
    private static final int TTL = 600;
    public static final int REGIST_INVITE_LIMIT = 5;
    private static final String CLUSTER_NAME = "im_small_redis";
    private volatile static RegistLimitRedisCache registLimitRedisCache;

    public static RegistLimitRedisCache getInstance() {
        if (registLimitRedisCache == null) {
            synchronized (RegistLimitRedisCache.class) {
                if (registLimitRedisCache == null) {
                    registLimitRedisCache = new RegistLimitRedisCache();
                }
            }
        }
        return registLimitRedisCache;
    }
